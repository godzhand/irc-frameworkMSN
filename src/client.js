...
module.exports = class IrcClient extends EventEmitter {
	...
	createStructure() {
		...
		client.connection.on("message", function (message, raw_line) {
			if (message.command == "800") {
				const argX = raw_line.split(" ");
				if (argX[2] == "*") {
					if (argX[3] == "1") {
						client.raw("AUTH GateKeeper I :GKSSP\0]\0\0\0\0\0\0");
					}
				}
				// client.raw("AUTH GateKeeper I :GKSSP\\0\\0\x03\\0\\0\\0\x01\\0\\0\\0\r\n");
				//client.raw("AUTH GateKeeper I :GKSSP\0]\0\0\0\0\0\0");
			}
			if (message.command == "AUTH") {
				const buffer = require("buffer");
				const latin1buffer = buffer.transcode(Buffer.from(raw_line), "utf8", "latin1");
				const latin1string = latin1buffer.toString("latin1");
				const commandBody = latin1string.substring(0);
				const argX = raw_line.split(" ");
				if (argX[2] == "S") {
					const execFile = require("child_process").execFile;
					let path = require("path");
					let reqPath1 = path.join(__dirname, "../../../MSNChatAuth/");
					const {exec} = require("child_process");
					const Util = require("util");
					const asyncExec = Util.promisify(execFile);

					asyncExec(reqPath1 + "auth.exe", [argX[0], argX[1], argX[2], argX[3], argX[4]])
						.then((stdout, stderr) => {
							//stdout.pipe(process.stdout);
							const authfile = require("node:fs");
							console.log(stdout);
							try {
								let path = require("path");
								let reqPath = path.join(__dirname, "../../../MSNChatAuth/auth.gk");
								const data = authfile.readFileSync(reqPath, "latin1");
								console.log(data);
								client.raw(data);
							} catch (err) {
								console.error(err);
					...
					// client.raw('NICK','>' + client.user.nick);
					client.raw("NICK >" + client.user.nick + "\nIRCX\r\njoin %#the\\blobby\r\n");
				}
			}
			client.raw_middleware.handle(
				[message.command, message, raw_line, client],
				function (err) {
					if (err) {
		...
		this.command_handler.on("all", function (event_name, event_arg) {
			...
			if (["privmsg", "notice", "action"].indexOf(event_name) > -1) {
				event_arg.reply = function (message) {
					const dest =
						event_arg.target === client.user.nick ? event_arg.nick : event_arg.target;
	...
	registerToNetwork() {
		...
		this.raw("MODE ISIRCX\r\nIRCVERS IRC3 MSN-OCX!9.02.0309.2602 en-us\r\n");
	...
	startPeriodicPing() {
		...
		function resetPingTimer() {
			...
			ping_timer = client.connection.setTimeout(
				pingServer,
				client.options.ping_interval * 1000
			);
		}
	...
	startPingTimeoutTimer() {
		...
		function resetPingTimeoutTimer() {
			...
			timeout_timer = client.connection.setTimeout(
				pingTimeout,
				client.options.ping_timeout * 1000
			);
		}
		...
		function pingTimeout() {
			...
			const end_msg = client.rawString(
				"QUIT",
				"Ping timeout (" + client.options.ping_timeout + " seconds)"
			);
